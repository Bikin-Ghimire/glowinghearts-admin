...
type Prize = {
  ...
  isPercentage: boolean | number // tolerate 0/1 coming in
...
export default function PrizesStep({
  ...
  const minDateTime = useMemo(
    () => (salesStartDate && salesStartDate.length >= 16 ? salesStartDate.slice(0, 16) : new Date().toISOString().slice(0, 16)),
    [salesStartDate]
  )
    ...
    <div className="text-zinc-900 dark:text-zinc-100">
      ...
      <div className="space-y-6">
        {prizes.map((prize, index) => {
          const isPercentBool = !!prize.isPercentage
            ...
            >
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="block w-full rounded-md border border-zinc-300 bg-white p-2 text-zinc-900
                               ...
                               dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100 dark:[color-scheme:dark]"
                  >
                ...
                <div className="mt-6 flex items-center">
                  <input
                    id={`isPercentage-${index}`}
                    type="checkbox"
                    checked={isPercentBool}
                    ...
                    className="mr-2 h-4 w-4 rounded border-zinc-300 text-indigo-600 focus:ring-indigo-600
                               dark:border-zinc-600 dark:bg-zinc-900 dark:checked:bg-indigo-500"
                  />
                  <label
                    htmlFor={`isPercentage-${index}`}
      ...
      {/* Claim Period */}
      <label className="mt-5 mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300">
        Prize Claim Period
      </label>
      <input
        type="number"
